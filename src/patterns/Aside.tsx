import { Card, CardContent, List, ListItemButton, ListItemText, CircularProgress, Typography } from '@mui/material'
...
const fetcher = async (url: string): Promise<string[]> => {
  ...
  const { data } = await axios.get(new URL(`proxy?url=${encodeURIComponent(url)}`, location.href).href, {
    headers: {
...
const Aside = () => {
  ...
  const url = useBusField<string>('url')
          ...
          {matchedData.length > 0 ? (
            <List>
              {matchedData.map(({ title, matched }, i) => (
                ...
                >
                  <ListItemText primary={title} sx={[matched && { color: '#1976d2' }]} />
                </ListItemButton>
              ))}
            </List>
          ) : (
